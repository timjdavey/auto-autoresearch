...
* **Parallel solutions**. This is the biggest structural departure from autoresearch. We allow Scientists to try multiple `train.py` solutions at once. This was a suggestion from the Supervisor but not something it had remit to implement. Interestingly it dramatically increased the Scientists tendency to explore different solution trees, rather than just exploiting branches. It also helped squeeze more iterations in the same study time. This was easily achievable since these problems are not GPU bound.
